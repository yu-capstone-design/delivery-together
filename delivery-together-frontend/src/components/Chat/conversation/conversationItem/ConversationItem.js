...
const ConversationItem = ({ conversation, isActive, props }) => {
  ...
  return (
    <div
      className={className}
      onClick={() => {
        props.history.push('/chatRoom/' + conversation.roomId);
      }}
    >
      ...
      <div className="conversation-message">{conversation.chat.lastText}</div>
    </div>
  );
};
...
function checkTime(tagetTime) {
  ...
  if (leftTime < 60) {
    return String(leftTime) + '초 전';
  }

  leftTime = parseInt(leftTime / 60);
  if (leftTime < 60) {
    return String(leftTime) + '분 전';
  }

  leftTime = parseInt(leftTime / 60);
  if (leftTime < 60) {
    return String(leftTime) + '시 전';
  }

  leftTime = parseInt(leftTime / 24);
  return String(leftTime) + '일 전';
}
